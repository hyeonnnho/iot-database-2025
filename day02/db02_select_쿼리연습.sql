...
-- 파이썬, C#등 프로그래밍 언어로 가져갈 땐, 컬럼 이름, 컬럼 갯수를 모두 파악해야 하기 때문에 아래와 같이 사용
...
-- 3-5 : 가격이 1000원이하 20000이하인 도서를 검색하시오.
SELECT *
  FROM Book
...
-- _ 특정위치의 1개의 문자가 일치할 때 
		-- _구 : 첫 글자는 상관없고, 두번째 글자가 구이며 뒤에 글이 더 있음.
SELECT *
  FROM Book
...
-- ASC(ending) DESC(ending) : ASC는 생략가능
SELECT *
  FROM Book
