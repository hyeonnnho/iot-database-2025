...
# 5. DB 처리 함수들 정의
# 5-1 showDatas() - DB 학생정보 데이블에 데이터를 가져와 TreeView에 표시
def showDatas():
    '''
    ...
    ### DB연결. 커넥션 객체생성 -> 커서 -> 쿼리 실행 -> 커서로 데이터 패치 -> 커서 종료 -> 커넥션 종료
    ...
    ## 가져온 데이터 트리뷰에 추가
    print(data)
    dataView.delete(*dataView.get_children())   # 최초, 중간에 showDatas() 호출 시 마다 트리뷰 클리어
...
def modData():
    ...
    if std_id == '':
        messagebox.showwarning('UPDATE', '수정할 데이터를 선택하세요!')
        return
...
def delData():
    ...
    if std_id == '':
        messagebox.showwarning('DELETE', '삭제할 데이터를 선택하세요!')
        return
...
ent1.config(state='readonly', foreground='blue', disabledforeground='blue') # 값을 입력 못하게 방지
...
tk.Button(root, text='추가', font=myFont, height=2, width=13, command=addData).place(x=30, y=130) # 추가 버튼
## 8-1 수정버튼에 modData() 함수를 연결
tk.Button(root, text='수정', font=myFont, height=2, width=13, command=modData).place(x=140, y=130) # 수정 버튼
## 9-1 삭제버튼에 delData() 함수를 연결
tk.Button(root, text='삭제', font=myFont, height=2, width=13, command=delData).place(x=250, y=130) # 삭제 버튼
